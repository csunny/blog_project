var Ajax = {
	...
	formateParameters: function(){
		...
		for (var pro in params){
			var paramsValue = params[pro];
			paramsArray.push(prom + "=" + paramsValue);
		}
		return paramsArray.join("&");
		// key1 = val1 & key2=val2
	},
	//状态改变的处理
	readystatuschange： function(xmlhttp){
		var returnValue;
	...
	request: function(options){
		...
		xmlhttp.onreadystatuschange = function(){
			ajaxObj.readystatuschange.call(ajaxObj, xmlhttp);
		};
...
function startBuy(obj){
	if (obj.errno == 0){
		...
		document.getElementById("item-time").style.backgraoundColor = 'green';
...
function start(){
	...
	var itemobj = itemobj.item;
	var itemid = itemobj.item;
	var uid = 888;

	Ajax.request({
		url: 'http://localhost/miao/second',
		params: {id: itemidm, userid: uid},
		type: "json",
		callback: startBuy
	});
...
function buyItem(){
	var buyurl = document.getElementById('mp-item-link').getAttribute('href');
	var data = document.getElementById('mp-item-link').getAttribute('data-click');
	var obj = eval("(" + data + ")")
	var itemid = obj.item;
